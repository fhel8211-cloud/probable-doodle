...
// Switched from Gemini to DeepSeek for better reasoning capabilities
...
// DeepSeek API Keys - Round Robin System
const DEEPSEEK_API_KEYS = [
  'sk-c4a657283deb49e7a6440876491e122d',
...
function getNextDeepSeekKey(): string {
  const key = DEEPSEEK_API_KEYS[currentKeyIndex];
  currentKeyIndex = (currentKeyIndex + 1) % DEEPSEEK_API_KEYS.length;
  console.log(`Using DeepSeek API key index: ${currentKeyIndex - 1 < 0 ? DEEPSEEK_API_KEYS.length - 1 : currentKeyIndex - 1}`);
  return key;
}

// DeepSeek API call function
async function callDeepSeekAPI(messages: any[], temperature: number = 0.1, maxTokens: number = 4000): Promise<string> {
...
// Extract questions from PDF page using DeepSeek
...
// Generate new questions for a topic using DeepSeek
...
Generate exactly ${count} question(s). Ensure highest quality and accuracy.`;
  ...
  try {
    ...
    const response = await callDeepSeekAPI(messages, 0.3, 3000);
...
// Generate solutions for PYQs using DeepSeek
...
Ensure accuracy and provide comprehensive explanations.`;
  ...
  try {
    ...
    const response = await callDeepSeekAPI(messages, 0.1, 3000);
...
// Comprehensive question validation using DeepSeek AI
export async function validateQuestionWithDeepSeekAI(question: any): Promise<{ isWrong: boolean; reason: string }> {
...
Be extremely thorough and accurate in your analysis.`;
  ...
  try {
    ...
    const response = await callDeepSeekAPI(messages, 0.1, 2000);
...
export { getNextDeepSeekKey, callDeepSeekAPI };